...
export default function DashboardPage() {
  ...
  return (
    <div className="p-6">
      {/* Hero Banner */}
      <div className="mb-8 overflow-hidden rounded-2xl bg-gradient-to-br from-teal-600 via-teal-500 to-emerald-500 px-8 py-8 text-white shadow-xl shadow-teal-500/15">
        <div className="flex items-center justify-between">
          <div>
            <div className="mb-2 flex items-center gap-2">
              <Sparkles className="h-4 w-4 text-teal-200" />
              <span className="text-xs font-medium text-teal-100">AI-Powered Recruitment</span>
            </div>
            <h1 className="text-2xl font-bold tracking-tight">
              Welcome to CaresLink
            </h1>
            <p className="mt-1.5 max-w-md text-sm text-teal-100">
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        ...
        <div className="glass rounded-2xl px-5 py-5">
          ...
          <p className="mb-3 text-[11px] text-teal-600">Last 30 days activity</p>
          <dl className="space-y-2">
            {[
              ...
              <div
                key={row.label}
                className="flex items-center justify-between rounded-xl bg-teal-50/30 px-3.5 py-2.5 transition-colors hover:bg-teal-50/50"
              >
                <dt className="flex items-center gap-2.5 text-xs text-teal-700">
                  <div className="rounded-lg bg-teal-100/50 p-1.5">
                    <row.icon className="h-3 w-3 text-teal-500" />
                  </div>
                  {row.label}
                </dt>
                <dd className="text-sm font-semibold text-teal-900">
                  {row.val ?? "—"}
                </dd>
              </div>
            ))}
        ...
        <div className="glass rounded-2xl px-5 py-5">
          ...
          <p className="mb-3 text-[11px] text-teal-600">Jump to common tasks</p>
          <div className="space-y-2">
            {[
              ...
              <Link
                key={action.label}
                href={action.href}
                className="group flex items-center justify-between rounded-xl bg-teal-50/30 px-3.5 py-3 transition-all hover:bg-teal-50/50 hover:shadow-sm"
              >
                <div>
                  <p className="text-xs font-medium text-teal-800">{action.label}</p>
                  <p className="text-[11px] text-teal-600">{action.desc}</p>
                </div>
                <ArrowRight className="h-3.5 w-3.5 text-teal-400 transition-all group-hover:translate-x-0.5 group-hover:text-teal-600" />
              </Link>
            ))}
          </div>
        </div>
