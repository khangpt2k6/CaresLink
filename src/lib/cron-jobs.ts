/**
 * Job logic for BullMQ workers.
 * Extracted from cron.ts so both workers and API routes can call them.
 */
...
/** Check for upcoming interviews and send 24h / 1h reminders + unconfirmed follow-ups */
export async function checkAndSendReminders() {
  ...
  if (sent24h > 0 || sent1h > 0) {
    console.log(`[BullMQ] Sent ${sent24h} 24h reminders, ${sent1h} 1h reminders`);
  }
...
export async function runFollowUpSequence() {
  ...
  for (const c of stale) {
    ...
    if (result.success) {
      ...
      console.log(`[BullMQ] Sent Day ${sendDay} follow-up to ${c.email}`);
    }
  }
}

/** Agent cron job — contact applicants + run AI agent for follow-ups/reminders */
export async function runAgentCronJob() {
  const { runAgent } = await import("./agent");
  const { sendBookingLinkToCandidate } = await import("./send-booking-link");
...
Provide a brief summary: how many follow-ups sent, how many reminders sent.`;
  ...
  await prisma.agentRun.create({
    data: { trigger: "cron", report: parts.join("\n\n"), startedAt, completedAt },
  });

  // TASK 3 — Self-healing: embed candidates/jobs missing embeddings
  try {
    const { embedCandidate, embedJob } = await import("./embeddings");
    ...
    if (candidatesEmbedded > 0 || jobsEmbedded > 0) {
      console.log(`[BullMQ] Embedding health check: embedded ${candidatesEmbedded} candidates, ${jobsEmbedded} jobs`);
    }
  } catch (e) {
    console.error("[BullMQ] Embedding health check error:", e);
  }

  console.log(`[BullMQ] Agent cron completed in ${(completedAt.getTime() - startedAt.getTime()) / 1000}s`);
}
